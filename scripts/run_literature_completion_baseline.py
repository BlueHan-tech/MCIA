...
This is deliberately separate from run_all_experiments.py.  Input values must
be normalized beforehand; for faithful SGMD-AAE reproduction use 240 x 12,
120-ms non-overlapping, raw-DB2 windows as described in the paper.
"""
...
def train_sgmd(train, test, ratio, epochs, batch_size, seed, device):
    ...
    for _ in range(epochs):
        for (batch,) in loader:
            target = batch.to(device).permute(0,2,1).unsqueeze(1); observed = torch.from_numpy(mask_like(batch.numpy(), ratio, seed+_)).to(device).permute(0,2,1).unsqueeze(1)
    ...
    observed = mask_like(test, ratio, seed+999); x = torch.from_numpy(test).to(device).permute(0,2,1).unsqueeze(1); m = torch.from_numpy(observed).to(device).permute(0,2,1).unsqueeze(1)
    pred = sgmd_complete(generator.eval(), x, m).squeeze(1).permute(0,2,1).cpu().numpy(); return pred, observed
...
def main():
    ...
    values = np.load(a.input).astype(np.float32); values = values[:a.max_samples]
    if values.ndim != 3: raise ValueError("Input .npy must have shape (N,T,C).")
    split = max(1, int(.8*len(values))); train, test = values[:split], values[split:]
    ...
    missing = observed < .5; report = {"method":a.method,"input_shape":list(values.shape),"train_test":"first_80_percent_random_order_as_supplied","missing_ratio":a.missing_ratio,"masked_metrics":scores(pred[missing],test[missing]),"masked_rme":relative_mean_error(pred[missing],test[missing])}
