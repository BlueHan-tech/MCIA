...
\subsection{DB3 Amputee Adaptation Status}
No DB3 subject-adaptation result is reported in the current manuscript.  The previous implementation synthetically hid portions of an amputee subject's own recording and used the unmasked recording as a reconstruction target.  That objective is not valid for genuinely unreliable residual-limb channels, so its checkpoints, augmentation outputs, and downstream comparisons are excluded.  A future DB3 adaptation experiment requires a separately specified no-ground-truth self-supervised objective and a protocol-valid evaluation.
